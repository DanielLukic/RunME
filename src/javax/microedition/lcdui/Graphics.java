...
public final class Graphics
{
    ...
    {
        if ( (aRGB24 >> 24) == 0 ) myGraphics.setColor( new Color( aRGB24, false ) );
        else myGraphics.setColor( new Color( aRGB24, true ) );
    }

    public final void setFont( final Font aFont )
    {
        if ( myFont != null ) myFont.attached_graphics = null;
        myFont = aFont;

        if ( aFont != null )
        {
            myGraphics.setFont( aFont.awt_font );
            aFont.attached_graphics = myGraphics;
        }
    }
    ...
    {
        if ( myFont == null || myFont.awt_font == null ) return;
    ...
    private final Point getAlignedPosition( final int aX, final int aY, final int aWidth, final int aHeight, final int aAlignment )
    {
        final int x;
        if ( (aAlignment & Graphics.LEFT) != 0 ) x = aX;
        else if ( (aAlignment & Graphics.RIGHT) != 0 ) x = aX - aWidth;
        else if ( (aAlignment & Graphics.HCENTER) != 0 ) x = aX - aWidth / 2;
        else throw new IllegalArgumentException();

        final int y;
        if ( (aAlignment & Graphics.TOP) != 0 ) y = aY;
        else if ( (aAlignment & Graphics.BOTTOM) != 0 ) y = aY - aHeight;
        else if ( (aAlignment & Graphics.VCENTER) != 0 ) y = aY - aHeight / 2;
        else throw new IllegalArgumentException();
